...
class FormeBaseInput extends FormeBase {
    constructor(type, form, name) {
        super(type || 'baseInput', form, name);
    ...
    _findCustomActions() {
        if (!this._actions) {
            ...
            return this._actions.filter(action => !action.special);
        }
    }
    ...
    _findSpecialAction(search) {
        if (!this._actions) {
            ...
            return this._actions.find(action => action.special && action.action === search);
        }
    }
