...
class FormeComponent extends FormeContainer {
    ...
    _buildChildren() {
        //in this method we are responsible for modifying all children/descendants depending on the state of the component!
        ...
        if (!this._elements || !this._elements.length) {
            ...
            return Promise.all(this._elements.map(element => new Promise((resolve, reject) => {
            ...
            //allow "read" operation on defaultValue of component
            .then(() => this._processRead(this._defaultValue))
            .then(read => {
                    ...
                    if (this._expose === null) {
                        ...
                        if (this._expose.length === 1) {
                            ...
                            if (element) {
                                ...
                                pointer[element._outputName] = read;
                            }
                        } else {
                            ...
                            for(let path of this._expose) {
                                ...
                                if (element) {
                                    ...
                                    const value = utils.object.find.path(read, element._ownPathSegments);
                                    if (value !== undefined) {
                                        const pointer = utils.object.add.path(defaultValue, element._ownGroupSegments);
            ...
            //apply things set in component to exposed elements
    ...
    _addCustomValidateHandler(callback, error) {
        if (!this._composing) {
            //when the component is not composing (a flag set by forme when the component is being passed to compose handlers)... we are valditing on teh user (exposed) level
