...
class FormeComponent extends FormeContainer {
    ...
    _buildChildren() {
        ...
        return new Promise((resolve, reject) => {
            if (this._elements && this._elements.length) {
                ...
                //apply things set in component to exposed elements todo: this should be converted to a less manual approach, but for now it works!
    ...
    _buildValuesSelf(options, depth=0) {
        if (options.overwrite && this._overwritten) {
            //the value has been overwritten internally
    ...
    _processElementStateChanges(handler, oldState, newState, overwrite) {
        //we dont use the exposed version of value when its a validate handler that was created during the compose!
        //This is because we assume that validate handlers created in .compose() are for dealing with internal values
        if (overwrite) {
            if (!utils.value.compare(oldState.value, newState.value)) {
                this._overwrite = newState.value;
            }
        } else {
            if (!this._exposed || handler instanceof ComponentComposeValidateHandlerCustom) {
                //if not exposed, act as container
                return super._processElementStateChanges(handler, oldState, newState, overwrite);
            } else {
    ...
    _nextComposeHandler(details, handlers, index=0) {
        if (handlers.length === 0) {
            ...
            return Promise.resolve(handlers[index](this, details))//<- it was teh responsibility of the builder of this handler to conform to (component, details) => {} pattern
    ...
    _createElementState(handler) {
        //we dont use the exposed version of value when its a validate handler that was created during the compose!
        //we check this when the validate handler is specifically inherited from ComponentComposeValidateHandlerCustom
        if (!this._exposed || handler instanceof ComponentComposeValidateHandlerCustom) {
            //if not exposed, act as container
            return super._createElementState(handler);
        } else {
            return {
    ...
    _convertElementValues(input, out) {
        if (!this._exposed) {
            ...
            if (!out) {
                ...
                //exposed on the components name!
