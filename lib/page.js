...
class FormePage extends FormeContainer {
    ...
    get _destination() {
        //page container doesn't need destination as it used when a form uses the same path for each page
        return null;
    }

    //private properties
    get _url() {
        const pageQueryString = this._form._isFirstPage(this)?null:this._name;//hide the page querystring value if its the first page!
        return this._form._makeUrl(this._form._currentDriver.url(), this._request._token, pageQueryString);
    }
