...
//functions
function sortMapNodesByOrder(a, b) {
    //sort by: order asc, index asc
    return a.order - b.order || a.index - b.index;
}

function buildTemplateMap(parent) {
    //create map of child names (in correct order) on this particular parent.
    //we are working with generated template variables here so have no notion of elements!
    if (parent.map === undefined) {
        if (!parent.children) {
            //dont have a map!
            parent.map = null;
        } else {
            ...
            //iterate all child keys in the parent
            let index = 0;
            for (let name of Object.keys(parent.children)) {
                const child = parent.children[name];

                //process order
                const orderNull = child.order === undefined || child.order === null || child.order === false;
                const order = orderNull?0:child.order;
                ...
                //recurse into template children. Only recurse into groups because each forme element is responsible for building its own map!
                //as groups don't actually have an object, we need to build them out!
                if (child.__formeClass === 'group') {
                    buildTemplateMap(child);
                }
            }

            //sort them!
            nodes.sort(sortMapNodesByOrder);

            //finalise map (we only need a list of names)
            parent.map = nodes.map(node => node.name);
        }
    }
...
class FormeBase {
    constructor(name) {
        ...
        this._order = null;//all elements have order of 0 unless otherwise specified!
        ...
        this._context = {};
        this._tags = null;//a list of tags
    ...
    _buildConfigurableMethods() {
        return {
            ...
            classNames: new FormeConfigurableMethodPointer('className'),

            //base.tag(string)
            tag: new FormeConfigurableMethod('tag', [
                new FormeConfigurableOverride([
                    new FormeConfigurableStrings(['tag', 'tags'], false),
                ], true),
            ], new FormeConfigurableExportArrayStrings('_tags')),
            tags: new FormeConfigurableMethodPointer('tag'),
            ...
            //base.order(int)
            order: new FormeConfigurableMethod('order', [
                new FormeConfigurableOverride([
                    new FormeConfigurableIntOrNull('order', false),
                ], true),
            ], new FormeConfigurableExportIntOrNull('_order')),
    ...
    _buildFinish(external) {
        return new Promise((resolve, reject) => {
            //optimize the tags to unique tags only!
            if (this._tags !== null) {
                this._tags = this._tags.filter((elem, pos, arr) => arr.indexOf(elem) === pos);
            }
    ...
    _buildTemplate(options, parent) {
        ...
        if (this._buildTemplateInclude(options)) {
            ...
            //create a template map (+chain parent)
            .then(parent => buildTemplateMap(parent))//note: this is not calling this._buildFoo! because we are not working on elements/base/containers/etc
            ...
            if (options.group) {
                ...
                if (segments && segments.length) {
                    ...
                    for (let index = 0; index < segments.length; index++) {
                        ...
                        if (!child) {
                            //container has responsibility to create the group!
                            child = parent.children[segment] = this.container._createGroup(segment, groupPath.join('.'));
                        }
    ...
    _buildTemplateMap(options, parent) {
    ...
    _findExecuteHandlers(type) {
        return this._executeHandlers.filter(handler => handler.constructor === type || (Array.isArray(type) && type.indexOf(handler.constructor) !== -1));
    }

    _findValidateHandlers(type) {
        return this._validateHandlers.filter(handler => handler.constructor === type || (Array.isArray(type) && type.indexOf(handler.constructor) !== -1));
    }

    _findProcessHandlers(type) {
        return this._processHandlers.filter(handler => handler.constructor === type || (Array.isArray(type) && type.indexOf(handler.constructor) !== -1));
    }
    ...
    _findTaggedDescendants(tags, or=true, out=[]) {
        //by default base has no descendants!
        return out;
    }
    ...
    tag(tags) {
        if (this.callingConfigureMethod('tag')) {
            //how many?
            let changed = false;
    ...
    order(order=null) {
        if (this.callingConfigureMethod('order')) {
            //weighting order used in template vars
            this._order = order;
    ...
    //public has api
    hasTag(tags, or=true, wildcard=true) {
        if (or) {
            return utils.tags.compare.or(this._tags, tags, wildcard);
        } else {
            return utils.tags.compare.and(this._tags, tags, wildcard);
        }
    }
