...
class FormeBase {
    ...
    get storage() {
        if (this.callingStartedInactive('storage')) {
    ...
    getErrors() {
        if (this.callingActiveMethod('getErrors')) {
            this._request._getErrorsWithPath(this.path).map(error => error.error);
        }
    }
