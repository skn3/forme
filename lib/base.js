...
const {FormeConfigurableMethod, FormeConfigurableOverride, FormeConfigurableString, FormeConfigurableObject, FormeConfigurableCallbacks} = require('./configurable');
...
class FormeBase {
    ...
    static get configurableMethods() {
        ...
        let cache = configurableMethodClassCache[this.name];

        //check if cache, otherwise create a new cache!
        if (cache === undefined) {
            //need to build!
            configurableMethodClassCache[this.name] = cache = this.buildConfigurableMethods();
        }
