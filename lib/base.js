...
function sortMapNodesByOrder(a, b) {
    //sort by: orderNull asc, order asc, index asc
    return b.orderNull - a.orderNull || a.order - b.order || a.index - b.index;
}
        ...
        if (!parent.children) {
            ...
            for (let name of Object.keys(parent.children)) {
                ...
                const node = {
                    ...
                    //sort properties (should all be integers!)
                    index: index++,
                    orderNull: child.order !== undefined && child.order !== null ? 0 : 1,
                    order: child.order,
                };
            ...
            //sort them!
            map.sort(sortMapNodesByOrder);

            //finalise map (we only need a list of names)
            parent.map = map.map(node => node.name);
        }
    }
