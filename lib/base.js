...
class FormeBase {
    ...
    _buildConfiguration(methods) {
        //split the methods into ordered and none ordered
        ...
        for(let name of Object.keys(methods)) {
            ...
            if (method instanceof FormeConfigurableExportExecuteHandler) {
                orderedMethods.push({
                    name: name,
                    method: method,
                })
    ...
    _clone(form) {
        ...
        for(let key of Object.keys(this)) {
            ...
            //ignore functions and _form
            if (typeof property !== 'function' && key !== '_form') {
                if (property === null) {
                    clone[key] = null;
                } else {
                    if (Array.isArray(property)) {
                        //array of properties
                        clone[key] = property.map(item => item instanceof FormeBase ? item._clone(form) : item);
                    } else {
                        clone[key] = property;
                    }
                }
    ...
    context() {
        ...
        if (arguments.length >= 1 && typeof arguments[0] === 'object') {
            if (this.callingConfigureMethod('context')) {
                //set by object keys
