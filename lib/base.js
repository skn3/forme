...
const ProcessHandlerEmpty = require('./handlers/process/processHandlerEmpty');
...
class FormeBase {
    ...
    _buildConfigurableMethods() {
        return {
            ...
            ], new FormeConfigurableExportProcessHandler(ProcessHandlerEmpty), methodPriority.afterConversion),
    ...
    _buildRead(out, external) {
        ...
        //run handlers
        .then(container => this._buildReadHandlers(container, external))
    ...
    _buildReadHandlers(out, external) {
        //always chain out!
        return Promise.resolve(out);
    }
    ...
    _processReadHandlers(value) {
        if (!this._readHandlers || this._readHandlers.length === 0) {
            return Promise.resolve(value);
        } else {
            return this._nextReadHandler(this._readHandlers, value);
        }
    }
    ...
    _nextReadHandler(handlers, value, index=0) {
        //slightly different signature because we want to chain the result!
        return Promise.resolve(this._executeReadHandler(handlers[index], value))
        .then(value => ++index === handlers.length ? Promise.resolve(value) : this._nextReadHandler(handlers, value, index));
    }
    ...
    empty(value) {
        if (this.callingConfigureMethod('empty')) {
            //add handler
            this._addProcessHandler(new ProcessHandlerEmpty(value));
