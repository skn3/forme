...
class FormeBase {
    constructor(name) {
        ...
        this._name = name;
        this._label = null;
        this._errorLabel = this._name;//we always need an error label!
        this._classNames = [];
        ...
        this._template = null;//not everything will use this
        this._context = {};

        this._hideLabel = false;
        this._hideDescendantLabels = false;

        this._alwaysInvalid = false;//allow this element to alwasy fail validation!
    ...
    _buildHandlerPropertyKeys() {
        return ['_buildHandlers', '_processHandlers', '_validateHandlers', '_invalidHandlers', '_validHandlers', '_executeHandlers', '_successHandlers', '_failHandlers', '_submitHandlers', '_actionHandlers', '_doneHandlers', '_setterHandlers'];
    }
    ...
    get _ownPathLength() {
        ...
        //other extended elements may not consider their name to be include,d such as pages!
    ...
    _buildChildren() {
        //by default base does not have children!
        return Promise.resolve();
    }
    ...
    _buildValues(options, parent) {
        ...
        if (this._buildValuesInclude(options)) {
            if (!options.group) {
                ...
                return this._buildValuesFlat(options, parent);
                ...
                return this._buildValuesStructure(options, pointer, value);
            }
        }
    ...
    _buildTemplateFinalise(options, parent) {
        ...
        if (!this._template) {
            ...
            if (this._templateClient) {
                ...
                return utils.promise.result(this._driver.renderTemplate(this._form, this, this._template, parent))
                .then(rendered => {
    ...
    _processExecuteHandler(handler) {
        ...
        const newState = this._createExecutionState(handler);
        const oldState = this._createExecutionState(handler);
        ...
        return utils.promise.result(handler.execute(this, newState))

        //process any state changes
        .then(() => this._processExecutionStateChanges(handler, oldState, newState))
    ...
    _executeSelf() {
        ...
        .then(() => {
            //check if always invalid or if any descendants flagged invalid!
            if (this._alwaysInvalid || this._request._hasInvalidDescendant(this.path)) {
                this._request._error = true;
        ...
        //invalid :(
        .catch(err => {
            //extra precaution, mark request with error!
            this._request._error = true;
    ...
    _findDescendantContinue(segments, index) {
        //check the current segment and determin how to continue in teh search
    ...
    _nextActionHandler(handlers, index=0) {
        return utils.promise.result(this._executeActionHandler(handlers[index]))
    ...
    _setValue(value, merge, setter) {
        //first we should see if there is a setter (but only if flagged as allowing this operation)
        if (!this._setValueSetter(value, merge, setter)) {
            //no setter, so what type of value is this?
            if (value && typeof value === 'object') {
                //object
                if (!this._setValueObject(value, merge, setter)) {
                    //element indiccated that it wasnt able to set an object value, so clear everything!
                    if (!merge) {
                        this._clearValue();
                    }
                }
            } else {
                if (!this._setValuePrimitive(value, merge, setter)) {
                    //element indiccated that it wasnt able to set a primitive value, so clear everything!
                    if (!merge) {
                        this._clearValue();
                    }
                }
    ...
    _setValueSetter(value, merge, setter) {
        //return true to indicate that this element handled the setting of the value with a setter
        if (setter && this._setterHandlers.length > 0) {
            //iterate all handlers in order
            for (let handler of this._setterHandlers) {
                if (this._executeSetterHandler(handler, value, merge) === true) {
                    //handled
                    return true;
                }
            }
    ...
    label(label) {
        if (this.callingConfigureMethod('label')) {
            ...
            this._errorLabel = label || this._outputName;//always need an error label!
    ...
    context() {
        if (arguments.length >= 1 && typeof arguments[0] === 'object') {
            ...
            for (let key of Object.keys(details)) {
                this._setContext(key, details[key], exposed);
            }

        } else if (arguments.length === 1) {
            //get by name
            const name = arguments[0];
            return this._getContext(name);
            ...
            this._setContext(name, value, exposed);
        }
        ...
        if (this.callingConfigureMethod('configure')) {
            if (info) {
                if (typeof info !== 'object' || Array.isArray(info)) {
                    ...
                    //as we are potentially dealing with percieved order of configure properties when defining (e.g. int comes before options) we should sort the keys based on this!
                    //build key info once so the search does not have to do a ton of lookups!
                    let index = 0;
    ...
    getOwnErrors() {
        if (this.callingActiveMethod('getOwnErrors')) {
