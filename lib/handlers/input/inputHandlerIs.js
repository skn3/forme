            ...
            switch (this.type) {
                ...
                case 'email':
                    if (!validator.isEmail(state.value)) {
                        return Promise.reject('{label} is not a valid email');
                    }
                    break;

                case 'username':
                    if (validator.whitelist(state.value, 'a-zA-Z0-9\_\-') != state.value) {
                        return Promise.reject('{label} is not a valid username');
                    }
                    break;

                case 'subdomain':
                    if (validator.whitelist(state.value, 'a-zA-Z0-9\_\-') != state.value) {
                        return Promise.reject('{label} is not a valid subdomain');
                    }
                    break;

                case 'isodate':
                    if (!validator.isISO8601(state.value)) {
                        return Promise.reject('{label} is not a valid date');
                    }
                    break;

                case 'text':
                    if (!validator.isAscii(state.value)) {
                        return Promise.reject('{label} is not a valid text');
                    }
                    break;
            }
        }
