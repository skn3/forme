...
class FormeContainer extends FormeBase {
    constructor(name) {
        ...
        this._groupTemplates = {};//a full group path lookup that contains a set of template vars to be populated when building the template group!
    ...
    _buildConfigurableMethods() {
        return Object.assign(super._buildConfigurableMethods(), {
            ...
            //container.groupTemplate(*multiple*)
            groupTemplate: new FormeConfigurableMethod('groupTemplate', [
                //container.groupTemplate(name, template, merge)
                new FormeConfigurableOverride([
                    new FormeConfigurableStrings(['group', 'name'], true),
                    new FormeConfigurableObject(['template', 'vars'], false),
                    new FormeConfigurableBool(['merge'], false),
                ], true, true),

                //container.groupTemplate(source)
                new FormeConfigurableOverride([
                    new FormeConfigurableObject(['group', 'groups', 'source'], true),
                ], true, true),
            ], new FormeConfigurableExportObject('_groupTemplates')),
            groupTemplates: new FormeConfigurableMethodPointer('groupTemplate'),
    ...
    _addGroupTemplate(path, template, merge) {
        if (path) {
            let existing = merge && this._groupTemplates[path] || null;
            if (!existing) {
                //replace
                this._groupTemplates[path] = Object.assign({}, template);
            } else {
                //merge
                utils.merge.allowOverwriteWithNull(this._groupTemplates[path], template);
            }
        }
    }
    ...
    _findDescendants(path) {
        const segments = utils.path.segments(path);//this wont create more, beyond the first recursion!
        const segmentsLength = segments.length;

        //if there are no segments in the path then we cant return SELF!
        if (!segments || segmentsLength === 0 || this._elements.length === 0) {
            return [];
        }

        //scan child elements
        const result = [];//build results!
        const stack = [{element: this, index:0}];//start stack off with self!

        let current = null;
        while(current = stack.pop()) {
            for (let element of current.element._elements) {
                //check to allows us to quickly skip certain scans
                if (current.index + element._groupLength <= segmentsLength) {
                    let invalidGroup = false;
                    let segmentIndex = current.index;
                    const groupLength = element._groupLength;
                    let groupIndex = 0;

                    //satisfy as much as possible of the group in the element
                    //we bascially keep going until the segmetns run out or the element group runs out! Each of these counts as a valid match!
                    if (groupLength) {
                        while (groupIndex < groupLength && segmentIndex < segmentsLength) {
                            if (segments[segmentIndex] !== element._group[groupIndex]) {
                                invalidGroup = true;
                                break;
                            }
                    ...
                    //only continue if we didnt find an invalid group
                    if (!invalidGroup) {
                        //have we found the match already?
                        if (segmentIndex === segmentsLength) {
                            //at the end of the segment chain so it must be a result!
                            //only allow elements that specify that the ycan be returned!
                            if (element._findDescendantFound()) {
                                result.push(element);
                            }
                        } else {
                            //continue searching but only if the entire group of the element was satisified (because we still have segments left to search)
                            if (groupIndex === groupLength) {
                                //the entire group of the element has been satisified so how should we continue!
                                const newSegmentIndex = element._findDescendantContinue(segments, segmentIndex);
                                if (newSegmentIndex !== false && newSegmentIndex !== segmentIndex) {
                                    //we can only continue if the element didnt halt and if the element changed the segment index! If we dont check this then we could get into an infinite loop!
                                    if (newSegmentIndex === segmentsLength) {
                                        //the element moved the search onto completion!
                                        if (element._findDescendantFound()) {
                                            result.push(element);
                                        }
                                    } else {
                                        //add this element to the stack for further investigation!
                                        stack.push({element: element, index: newSegmentIndex});
                                    }
                                }
                            }
                        }
    ...
    _findTaggedDescendants(tags, or=true, wildcard=true, out=[]) {
        for(let element of this._elements) {
            //element
            if (element.hasTag(tags, or)) {
                out.push(element);
            }

            //recurse
            element._findTaggedDescendants(tags, or, wildcard, out);
        }
    ...
    _createGroup(name, path) {
        //let super handle standard group creation
        return Object.assign({}, this._groupTemplates[path], super._createGroup(name, path));
    }
    ...
    groupTemplate() {
        //add template vars to a particular group
        if (this.callingConfigureMethod('groupTemplate')) {
            if (arguments.length) {
                if (typeof arguments[0] === 'object') {
                    if (!Array.isArray(arguments[0])) {
                        //defined as object of { groupPath: template, ... }
                        const source = arguments[0];
                        const merge = !!arguments[1];
                        for (let path of Object.keys(source)) {
                            const template = source[path] || null;
                            this._addGroupTemplate(path, template, merge);
                        }
                        
                    } else {
                        //multiple paths
                        const paths = arguments[0];
                        const template = arguments[1] || null;
                        const merge = !!arguments[2];
    ...
    groupTemplates() {
        return this.groupTemplate.apply(this, arguments);
    }
    ...
    getElements(path) {
        //attempt to find multiple descendants
        return this._findDescendants(path);
    }
    ...
    getElementContext(path, context) {
        const element = this._findDescendant(path);
        if (!element) {
            return undefined;
        }
    ...
    getElementsContext(path, context) {
        return this._findDescendants(path).map(element => element.getContext(context));
    }

    getTaggedElements(tags, or=true, wildcard=true) {
        return this._findTaggedDescendants(tags, or, wildcard);
    }

    getTaggedElementValuesGrouped(tags, or=true, wildcard=true) {
        if (this.callingActiveMethod('getTaggedElementValuesGrouped')) {
            const elements = this._findTaggedDescendants(tags, or, wildcard);
            const values = {};
            for (let element of elements) {
                const pointer = utils.object.add.path(values, element._containerPathSegments);
                pointer[element._outputName] = element.getValue();
            }
            return values;
        }
    }

    getTaggedElementValuesTrimmed(tags, trim, or=true, wildcard=true) {
        if (this.callingActiveMethod('getTaggedElementValuesTrimmed')) {
            const elements = this._findTaggedDescendants(tags, or, wildcard);
            const values = {};
            for (let element of elements) {
                let segments = element._containerPathSegments;

                //do we need to trim the segments? (trim should always be provided as array of paths)
                if (trim) {
                    //make sure trim is an array of string paths
                    if (typeof trim === 'string') {
                        trim = [trim];
                    }

                    //only bother processing trim if its an array of segments
                    if (Array.isArray(trim) && trim.length) {
                        //keep trying all trim paths provided until we have found the path with the deepest segment index!
                        let sliceIndex = 0;

                        for(let trimPath of trim) {
                            const trimSegments = utils.path.segments(trimPath);

                            if (segments.length >= trimSegments.length && trimSegments.length > sliceIndex) {
                                let matched = true;
                                for (let index = 0; index < trimSegments.length; index++) {
                                    if (trimSegments[index] !== segments[index]) {
                                        matched = false;
                                        break;
                                    }
                                }
                                ...
                                if (matched) {
                                    sliceIndex = trimSegments.length;
                                }
                            }
                        }

                        //slice it (if we need to do so)
                        if (sliceIndex > 0) {
                            segments = segments.slice(sliceIndex);
                        }
                    }
                }

                //create object structure and dump teh value in it!
                const pointer = utils.object.add.path(values, segments);
                pointer[element._outputName] = element.getValue();
            }
            return values;
        }
    ...
    getTaggedElementValuesFlattened(tags, or=true, wildcard=true) {
        if (this.callingActiveMethod('getTaggedElementValuesFlattened')) {
            const elements = this._findTaggedDescendants(tags, or, wildcard);
            const values = {};
            for (let element of elements) {
                values[element._outputName] = element.getValue();
            }
            return values;
        }
    }
    ...
    setElementContext(path, ...params) {
        const element = this._findDescendant(path);
        if (!element) {
            return undefined;
        }
        return element.getContext.apply(element, params);
    }
