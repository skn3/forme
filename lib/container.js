...
class FormeContainer extends FormeBase {
    ...
    _nextValidateHandler(index) {
        ...
        const lookup = {};
        const state = {
            values: this._form._fetchValues(false, true, false, true, lookup, null),
        };
        const oldValues = this._form._fetchValues(false, false, false, true, null, null);
            ...
            .then(() => {
                ...
                for (let input of this._form._inputs) {
                    ...
                    if (newValue != oldValues[input._name]) {
                        this._form.value(input, newValue);
                    }
                }
    ...
    next(callback) {
        if (utils.call.check.not.active(this._form, this._containerType+'.store()')) {
            return this.action(constants.actions.next, callback);
        }
    }

    prev(callback) {
        if (utils.call.check.not.active(this._form, this._containerType+'.store()')) {
            return this.action(constants.actions.prev, callback);
        }
    }
