...
class FormeContainer extends FormeBase {
    ...
    _nextValidateHandler(index) {
        ...
        .catch(err => {
            ...
            //need to catch the original error in debug mode
            //because otherwise it wont appear to have happened
            //_catchError will filter out any forme errors
            if (constants.dev) {
    ...
    load(callback) {
        if (utils.call.check.not.active(this, this._containerType+'.submit()')) {
