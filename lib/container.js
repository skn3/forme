...
class FormeContainer extends FormeBase {
    ...
    _createGroup(name, path) {
        ...
        const group = super._createGroup(name, path);

        //find template and add it to the group!
        group.template = this._groupTemplates[path] || {};
    ...
    getTaggedElementValuesGrouped(tags, or=true, wildcard=true) {
        if (this.callingActiveMethod('getTaggedElementValues')) {
