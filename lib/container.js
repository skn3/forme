...
class FormeContainer extends FormeBase {
    ...
    _buildConfigurableMethods() {
        return Object.assign(super._buildConfigurableMethods(), {
            ...
            element: new FormeConfigurableMethod('element', [
                ...
                new FormeConfigurableOverride([
                    new FormeConfigurableString(['element'], true),
                    new FormeConfigurableString(['name'], true),
    ...
    _createElement(element, details) {
        switch(element) {
            case 'input':
                return this._createInput(details);
            case 'component':
                return this._createComponent(details);
            default:
                throw new FormeError(`unknown element type '${element}'`);
        }
    }
    ...
    _getNamedValue(name, defaultValue=null, unsafe=false) {
        ...
        if (this._form._isBuilding()) {
            ...
            if (!input) {
                if (unsafe) {
                    return this._request._getNamedValue(name, defaultValue, unsafe);//pass in teh unsafe flag and let the request deal with where to read from!
                } else {
                    return defaultValue;
                }
            }
    ...
    element() {
        ...
        if (this.callingConfigureMethod('component')) {
            if (arguments.length === 1) {
                ...
                if (Array.isArray(details)) {
                    //multiple
                    return details.map(element => this._addElement(this._createElement(element.element, element)));
                } else {
                    //single
                    return this._addElement(this._createElement(details.element, details))
                }
    ...
    addElementError(path, error) {
        if (this.callingActiveMethod('addElementError')) {
            ...
            if (element) {
                ...
                //lost
                this._form._pipeLostError(error, element.formeClass, element.path, element._name);
            }
        }
    }
