...
class FormeContainer extends FormeBase {
    ...
    _processExecutionStateChanges(handler, oldState, newState) {
        ...
        for (let element of this._elements) {
            ...
            const newValue = utils.structure.find.path(newState.value, element._ownPathSegments, null);
            const oldValue = utils.structure.find.path(oldState.value, element._ownPathSegments, null);
    ...
    _getNamedValue(name, defaultValue=null, unsafe=false) {
        ...
        if (this._form._isBuilding()) {
            ...
            const value = this._form._readValue(name, this._requestFirst, this._request._submit, false, true, input?input._permanentValue:null, input?input._overrideValue:null, null, null);
