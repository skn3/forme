...
class FormeContainer extends FormeBase {
    ...
    _buildComponents(components) {
        ...
        if (components === null || components.length === 0) {
            ...
            return Promise.all(components.map(component => {
                ...
                //driver handler
                handlers.push(() => this._form._driver.compose(this._form, component, details));
                ...
                .then(() => {
                    ...
                    if (inputs && inputs.length) {
                        ...
                        for (let input of inputs) {
                            const clone = input._clone(component._form, component._page);
                            ...
                            if (component._name) {
                                clone.group(component._name, false);
                            }

                            //now add the input to the components container
                            component.container._addInput(clone);
                        }
                    }
                });
            }));
    ...
    _nextComposeHandler(component, details, handlers, index=0) {
        if (handlers.length === 0) {
            ...
            .then(halt => halt === true || ++index === handlers ? Promise.resolve() : this._nextComposeHandler(component, details, handlers, index));
        }
    }
    ...
    _validateAddInput(name) {
        ...
        return this._addInput(new this._form._driverClass.inputClass(this._form, this._page, name)).configure(configure);
    }

    _validateAddComponent(name, type=undefined) {
        //get the details when passed in
        let configure = null;
        if (name && typeof name === 'object') {
            //passed as configure object, copy it so we dont kill the passed in data
        ...
        //add
        //noinspection Annotator
        return this._addComponent(new this._form._driverClass.componentClass(this._form, this._page, name, type)).configure(configure);
    }
    ...
    add(details) {
        if (this.callingConfigureMethod('add')) {
            ...
            if (Array.isArray(details)) {
                //multiple
                return details.map(details => this._validateAddInput(details));
            } else {
                //single
                return this._validateAddInput(details);
            }
        }
    }
        ...
        if (this.callingConfigureMethod('component')) {
            if (arguments.length === 1) {
                ...
                if (Array.isArray(details)) {
                    ...
                    return details.map(details => this._validateAddComponent(details));
                } else if (typeof details === 'object') {
                    //single configure object (to be validated in _component())
                    return this._validateAddComponent(details);
                }
            } else if (arguments.length >= 2) {
                //passed as params
                return this._validateAddComponent(...arguments);
            }
        }
    }
