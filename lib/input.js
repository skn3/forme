...
class FormeInput extends FormeBase {
    ...
    _buildConfigurableMethods() {
        return Object.assign(super._buildConfigurableMethods(), {
            ...
            //input.optionsWhitelist(*multiple*)
            optionsWhitelist: new FormeConfigurableMethod('options', [
                //input.optionsWhitelist(array)
                new FormeConfigurableOverride([
                    new FormeConfigurableArray(['options', 'option', 'items', 'item', 'value', 'values'], true),
                ], true),

                //input.optionsWhitelist(object)
                new FormeConfigurableOverride([
                    new FormeConfigurableObject(['options', 'option', 'items', 'item', 'value', 'values'], true),
                ], true),

                //input.optionsWhitelist(string)
                new FormeConfigurableOverride([
                    new FormeConfigurableString(['options', 'option', 'items', 'item', 'value', 'values'], true),
                ], true),
            ], new FormeConfigurableExportArrayKeyValues('_options', 'label', 'value')),
    ...
    _buildReadSelf(out, external) {
        ...
        const submit = this._requestSubmit;

        //read raw and value both at the same time!
        return Promise.all([
            this._readValueFromEverywhere(first, submit, true, false, null, external),//raw values cant read from store
            this._readValueFromEverywhere(first, submit, false, !submit, null, external)//can only read from the store when NOT submitting
        ])

        //save into out
        .then(results => {
            out.raw[this._name] = results[0];
            out.values[this._name] = results[1];
    ...
    _buildReadHandlers(out, state, external) {
        //input only modifies the out.values entry that belongs to self!
        return this._processReadHandlers(out.values[this._name], state)
        .then(value => {
            out.values[this._name] = value;
    ...
    _buildValuesSelf(options, depth=0) {
        //all we need to do is return the inputs current value... the structure is dictated by the calling parent!
    ...
    _buildTemplateVars(options) {
        ...
        //size
        const size = this._calculateMinMax();
        vars.min = size.min;
        vars.max = size.max;
    ...
    _calculateMinMax() {
        ...
        let handlers = this._findValidateHandlers([InputValidateHandlerSize, InputValidateHandlerMin, InputValidateHandlerMax]);
        if (handlers.length) {
            for(let handler of handlers) {
                if (handler.min !== null) {
                    out.min = out.min === null ? handler.min : Math.min(out.min, handler.min);
                }
                
                if (handler.max !== null) {
                    out.max = out.max === null ? handler.max : Math.max(out.max, handler.max);
                }
            }
        }
    ...
    _readValueFromEverywhere(first, submit, raw, store, defaultValue, external) {
        //depending on submit or first we modify the default value to that of the input
        return new Promise((resolve, reject) => resolve(this._form._readValueFromEverywhere(this._name, first, submit, raw, store, this._permanentValue, this._overrideValue, !submit && first ? this._defaultValue || defaultValue : defaultValue, external)));
    }
    ...
    _setValueObject(value, merge, setter) {
        //just use the primitive version!
        return this._setValuePrimitive(value, merge, setter);
    }
    ...
    getValue() {
        ...
        return this._processGetters(this._form._getNamedValue(this._name));
    }
