...
class Input {
        ...
        return new Promise((resolve, reject) => {
            ...
            .then(() => {
                ...
                if (state.value !== oldValue) {
                    this._form.value(req,this,state.value);
                }
            ...
            .catch(err => {
                ...
                //cancel iteration
                reject(new FormeInputError(error, this._name));
            });
        })
        .then(() => ++index == this._validateHandlers.length ? Promise.resolve() : this._validateNextHandler(req, index, state));
    }
    ...
    _submitNextHandler(req, index) {
        ...
        //current submit
        return utils.promise.result(handler.call(this, req, this.form, this))
        .then(() => ++index == this._submitHandlers.length ? Promise.resolve() : this._submitNextHandler(req, index));
    }
    ...
    //request api
    error(req, message) {
        //shortcut to add a message (form handles input piping)
        this._form.error(req, this._name, message);
    }

    value() {
        if (arguments.length > 0) {
            if (this._form._containsRequest(arguments[0])) {
                if (arguments.length == 1) {
                    //get value
