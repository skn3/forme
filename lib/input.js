...
//functions
function createInputExecutionState(value) {
    return {
        require: false,
        value: value,
    };
...
class FormeInput extends FormeBase {
    ...
    _buildConfigurableMethods() {
        return Object.assign(super._buildConfigurableMethods(), {
            ...
            ], new FormeConfigurableExportValidateHandler(InputValidateHandlerOptions), methodPriority.validation),
    ...
    get _requestValue() {
        if (this._form._currentRequest) {
            return this._form._currentRequest._getValue(this._name, undefined, false);
        } else {
            return undefined;
        }
    }
    ...
    _buildValuesSelf(options) {
        ...
        if (options.raw) {
            ...
            value = this._request._getValue(this._name, value, false);
        }
    ...
    _buildTemplateVars(options) {
        ...
        const checked = (first && this._checked) || (!first && ((type === 'checkbox' && value !== null) || (type !== 'checkbox' && value !== null)));
    ...
    _createExecutionState(value) {
        return createInputExecutionState(this._form._getNamedValue(this._name));
    }

    //private get methods
    _getProcessedValue(value) {
        //allows for reading value from the input even when it hasnt submitted!
        const state = createInputExecutionState(value);
    ...
    options(options, error) {
        if (this.callingConfigureMethod('options')) {
            ...
            //add handler
            this._addValidateHandler(new InputValidateHandlerOptions(this._options, error));
