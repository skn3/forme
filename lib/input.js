...
class FormeInput extends FormeBase {
    ...
    _buildConfigurableMethods() {
        return Object.assign(super._buildConfigurableMethods(), {
            ...
            match: new FormeConfigurableMethod('match', [
                ...
                new FormeConfigurableOverride([
                    new FormeConfigurableString(['path', 'target', 'name'], true),
                    new FormeConfigurableBool('strict', true),
                    new FormeConfigurableString('error', true),
                ], false),
    ...
    _buildValues(options, parent) {
        ...
        if (this._buildValuesInclude(options)) {
            ...
            if (options.store) {
                ...
                value = this._request._getStoreValue(this._pathSegments, defaultValue);
            } else {
                value = defaultValue;
            }
            ...
            if (options.raw) {
                value = this._request._getRawValue(this._name);
            } else {
                value = this._request._getValue(this._name);
            }

            //build groups (might not build any, depending on options)
            const pointer = this._buildValueGroups(options, parent);

            //add value to the pointer
            if (options.alias) {
                pointer[this._outputName] = value;
            } else {
                pointer[this._name] = value;
            }
        }
    ...
    _buildTemplateVars(options) {
        ...
        //error (state)
        if (errors && errors.length && this._form._errorClassNames.length) {
        ...
        return Object.assign(super._buildTemplateVars(options), {
            ...
            value: this._requestValue,
            checked: (this._requestFirst && this._checked) || (!this._requestFirst && ((type === 'checkbox' && this._requestValue !== null) || (type !== 'checkbox' && this._requestValue !== null))),
    ...
    match(path, strict, error) {
        if (this.callingConfigureMethod('match')) {
            //validate the value against various value types
            this._addValidateHandler(new InputValidateHandlerMatch(path, strict, error));
