...
class FormeInput extends FormeBase {
    ...
    _nextValidateHandler(index) {
        ...
        //process handler and make sure to get up-to-date value from container
        return new Promise((resolve, reject) => {
            ...
            .catch(err => {
                ...
                //need to reject otherwise iteration wont continue
                reject();
            });
        })
    ...
    is(type, options, error) {
        if (utils.call.check.not.active(this._form, 'input.is()')) {
            ...
            this._validateHandlers.push(new InputHandlerIs(type, options, error));
    ...
    bool(allowNull) {
        if (utils.call.check.not.active(this._form, 'input.bool()')) {
            //add handler
            this._validateHandlers.push(new InputHandlerConvert('bool', allowNull));
    ...
    int(allowNull) {
        if (utils.call.check.not.active(this._form, 'input.int()')) {
            ...
            //add handler
            this._validateHandlers.push(new InputHandlerConvert('int', allowNull));
    ...
    string(allowNull) {
        if (utils.call.check.not.active(this._form, 'input.string()')) {
            ...
            //add handler
            this._validateHandlers.push(new InputHandlerConvert('string', allowNull));
    ...
    float(allowNull) {
        if (utils.call.check.not.active(this._form, 'input.float()')) {
            ...
            //add handler
            this._validateHandlers.push(new InputHandlerConvert('float', allowNull));
