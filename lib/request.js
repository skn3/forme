...
class FormeRequest {
    ...
    _load() {
        if (this._form._currentDriver) {
            ...
            .then(() => this._form._currentDriver.prune(this._form._sessionPrune))//prevent spamming of form sessions
        ...
        if (this._form._currentDriver) {
            if (this._reset) {
                ...
                return this._form._currentDriver.save(this._token, {
                    raw: this._form._buildValues({
                        secure: true,
                        raw: true,
                        special: true,
                    }),
                    values: this._form._buildValues({
                        secure: true,
                        special: true,
                    }),
                    errors: this._errors,
    ...
    _fetchOrphanedErrors() {
        //gets all erros that dont currently have an owner!
        return this._fetchCurrentErrors().filter(error => this._form._inputs.find(input => input._name === error.name) === undefined)
    }

    _fetchNamedErrors(name) {
        return this._fetchCurrentErrors().filter(error => error.name === name);
    }
    ...
    _getTokenInfo() {
        return new Promise((resolve, reject) => {
            let token = this._form._currentDriver.request(this._form._tokenField, undefined);
    ...
    _getValue(name, defaultValue=undefined) {
