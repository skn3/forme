...
class Forme extends FormeContainer {
    ...
    _wrap(storage) {
        ...
        form._driver = new form._driverClass(form, storage, form._request);
    ...
    _buildRequest(submit) {
        ...
        let page = this._findPageDestination(this._driver.url());
        if (page !== null) {
            this._request._page = page;
        }

        //override with get/post page
        page = this._driver.request(this._pageField);
        if (page !== undefined) {
    ...
    values(pages) {
        if (utils.call.check.active(this, 'form.values()')) {
            return this._fetchValues(false, true, false, !!pages, null, null);
        }
    }
