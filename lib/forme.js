...
const FormeInputError = require('./errors.js').FormeInputError;
const FormeSessionHandler = require('./session.js');
const Input = require('./input.js');

const FormHandlerRequire = require('./handlers/form/formHandlerRequire.js');
const FormHandlerValidate = require('./handlers/form/formHandlerValidate.js');
...
class Forme {
    ...
    _findGroupName(source) {
        if (source instanceof Input) {
            return source._grouped?source._groupName:false;
        } else {
            for (let index = 0; index < this._inputs.length; index++) {
                if (this._inputs[index]._grouped && this._inputs[index]._groupName == source) {
                    return this._inputs[index]._groupName;
                }
            }
            return false;
        }
    ...
    _buildTemplate(reqForm) {
        ...
        for (let inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
            ...
            if (typeof template.input[input._groupName] == 'undefined') {
                //first
                if (input._grouped) {
                    //input is forced into group, when group is defined (so always, but we should check here first)
                    template.input[input._groupName] = {};
                    template.input[input._groupName][buildInput.name] = buildInput;
                } else {
                    //nope single input
                    template.input[input._groupName] = buildInput;
                }
            } else {
                if (!(template.input[input._groupName] instanceof Object)) {
                    //convert to group
                    const oldInput = template.input[input._groupName];
                    template.input[input._groupName] = {};
                    template.input[input._groupName][oldInput.name] = oldInput;
                }
        ...
        for (let index = 0;index < this._inputs.length;index++) {
            ...
            if (!secure || !input._secure) {
                ...
                if (!group) {
                    ...
                    //group values
                    if (typeof groupState[input._groupName] == 'undefined') {
                        //do we force this into group because of input?
                        if (input._grouped) {
                            //forced into group (always happens for input.group)
                            values[input._groupName] = {};
                            values[input._groupName][input._name] = value;
                            groupState[input._groupName] = 2;

                            //add to lookup
                            if (lookup) {
                                lookup[input._name] = [input._groupName, input._name];
                            }
                        } else {
                            //single value
                            values[input._groupName] = value;
                            groupState[input._groupName] = 1;

                            //store the input name incase we need it later to convert single into array
                            singleInputName[input._groupName] = input._name;

                            //add to lookup
                            if (lookup) {
                                lookup[input._name] = [input._groupName];
                            }
                        }
                    } else {
                        //array value
                        if (groupState[input._groupName] == 1) {
                            //convert to group
                            const oldValue = values[input._groupName];
                            values[input._groupName] = {};
                            values[input._groupName][singleInputName[input._groupName]] = oldValue;
                            groupState[input._groupName] = 2;

                            //add to lookup
                            if (lookup) {
                                lookup[input._name] = [input._groupName, singleInputName[input._groupName]];
                            }
                        }
