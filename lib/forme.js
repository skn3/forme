...
class Forme extends FormeContainer {
    ...
    _build(view, submit, values) {
        return this._buildSteps(view, submit, values, [
            ...
            this._buildForm,
            this._buildPage,
            this._buildComponentsProcessQueue,
    ...
    _buildSteps(view, submit, values, steps) {
        ...
        return steps.reduce((prev, curr) => {
            ...
            .catch(err => {
                error = err || this._createError('unspecified error');
            });
        }, Promise.resolve())
        .then(() => {
            if (error) {
    ...
    _buildPageComponents() {
        return this._request._page === null? Promise.resolve() : this._request._page._buildComponentsProcessQueue();
    }
    ...
    _executeComposeHandler(component, details, handler) {
        return handler.call(this, this, this._request._page, this._request._page || this, component, details);
    }
    ...
    //templating
    template() {
        if (this.callingActiveMethod('template')) {
            const template = {
                form: {
                    name: this._name,
                    method: this._method,
...
module.exports.FormeConfigurableExportValidateHandlersConcat = FormeConfigurableExportValidateHandlersConcat;
