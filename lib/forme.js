    ...
    view(req, callback) {
        ...
        this._process(req, false, function(error){
            if (error) {
                ...
                callback.call(that, false, error);
            } else {
                //success
                if (callback != null) {
                    callback.call(that, true, null);
                }
            }
        });
    }
    ...
    validate(req, callback) {
        ...
        this._process(req, true, function(error) {
            if (error) {
                //error processing form
                if (callback != null) {
                    callback.call(that, false, null, error);
                }
            } else {
                ...
                async.eachSeries(that._inputs, function (input, next) {
                    ...
                    async.eachSeries(that._handlers, function (handler, next) {
                        ...
                        if (req.forme[that._name].errors.length > 0) {
                            ...
                            that.store(req, function (error) {
                                if (error) {
                                    ...
                                    if (callback != null) {
                                        ...
                                        errors.push(error);
                                        callback.call(that, false, that.values(req, false, true, false), errors);
                                    }
                                } else {
                                    //session saved
                                    if (callback != null) {
                                        callback.call(that, false, that.values(req, false, true, false), req.forme[that._name].errors);
                                    }
                                }
                            });
                        } else {
                            //success
                            if (callback != null) {
                                callback.call(that, true, that.values(req, false, true, false), null);
                            }
                        }
                    });
                });
