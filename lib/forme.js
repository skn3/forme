...
class Forme extends FormeContainer {
    ...
    _makeDestination() {
        ...
        //calculate details
        const pageQuery = this._pages === null || this._request._future === null || this._request._page._destination !== null;
        let destination = this._request._destination === null?this._driver.url():this._request._destination;
        let addQuery = null;
        const removeQuery = [];

        //token
        if (this._request._token === null) {
            removeQuery.push(this._tokenField);
        } else {
            addQuery = addQuery || {};
            addQuery[this._tokenField] = this._request._token;
        }
        ...
        if (pageQuery) {
            ...
            addQuery = addQuery || {};
            addQuery[this._pageField] = this._request._future;
        }
