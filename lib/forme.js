...
class Forme extends FormeContainer {
    ...
    _build(values) {
        ...
        .then(() => this._buildSteps(values, [
            //request
            values => this._buildRequest(values),

            //page
            values => this._buildImportPage(values),

            //load
            values => this._buildLoadHandlers(values),

            //build
            values => this._buildBuildHandlers(values),

            //children
            values => this._buildChildren(values),

            //finalise
            values => this._buildRead(values),
        ]))
        .then(() => this._buildFinish());
    }

    _buildSteps(values, steps) {
        //finished building
        ...
        return steps.reduce((prev, curr) => {
            ...
            return prev.then(() => !error?curr.call(this, values):Promise.resolve())
    ...
    _buildRead(values) {
        ...
        //read values from all inputs at once!
        return Promise.all(this._inputs.map(input => {
            return Promise.all([
                this._readInputValue(input, this._requestFirst, this._currentRequest._submit, true, false, null, values),//raw values cant read from store
                this._readInputValue(input, this._requestFirst, this._currentRequest._submit, false, !this._currentRequest._submit, null, values)//can only read from the store when NOT submitting
            ])
            .then(results => {
                const [raw, value] = results;
                buildRaw[input._name] = raw;
                buildValues[input._name] = value;
            });
        }))
    ...
    _readInputValue(input, first, submit, raw, store, defaultValue, values) {
        return new Promise((resolve, reject) => {
            ...
            if (input === null) {
                ...
                return resolve(this._readValue(input._name, first, submit, raw, store, input._permanentValue, input._overrideValue, !submit && first ? input._defaultValue || defaultValue : defaultValue, values));
            }
        })
        ...
        .then(value => {
            if (raw || !input) {
                //its a raw value so dont touch it!
                return value;
            } else {
                //its not raw and we have an input so lets feck with it!
                return input._processRead(value);
            }
        })
    }
