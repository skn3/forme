...
class Forme {
    ...
    processValues(req, secure, group, raw) {
        //build values
        const values = {};
        const groupState = {};
    ...
    view(req, callback) {
        ...
        this._process(req, false, function(error){
            if (error) {
                ...
                callback.call(that, req, that, false, error);
            } else {
                //success
                if (callback != null) {
                    callback.call(that, req, that, true, null);
                }
            }
        });
    }
    ...
    validate(req, callback) {
        ...
        this._process(req, true, function(error) {
            if (error) {
                //error processing form
                if (callback != null) {
                    callback.call(that, req, that, false, null, error);
                }
            } else {
                            ...
                            that.store(req, function (error) {
                                if (error) {
                                    ...
                                    if (callback != null) {
                                        ...
                                        errors.push(error);
                                        callback.call(that, req, that, false, that.processValues(req, false, true, false), errors);
                                    }
                                } else {
                                    //session saved
                                    if (callback != null) {
                                        callback.call(that, req, that, false, that.processValues(req, false, true, false), req.forme[that._name].errors);
                                    }
                                }
                            });
                        } else {
                            //success
                            //fire all input submit handlers
                            async.eachSeries(that._inputs, function (input, nextInput) {
                                //process submit handlers
                                if (input._submit.length == 0) {
                                    nextInput();
                                } else {
                                    async.eachSeries(input._submit, function (submitCallback, nextCallback) {
                                        //fire submit handler with finish callback
                                        submitCallback.call(input, req, that, input, function(){
                                            nextCallback();
                                        });
                                    }, function(){
                                        nextInput();
                                ...
                                //fire all form submit handlers
                                async.eachSeries(that._submit, function (submitCallback, nextCallback) {
                                    //fire submit handler with finish callback
                                    submitCallback.call(that, req, that, function(){
                                        nextCallback();
                                    });
                                }, function(){
                                    //fire success for validate
                                    if (callback != null) {
                                        callback.call(that, req, that, true, that.processValues(req, false, true, false), null);
                                    }
                                });
                            });
                        }
    ...
    store(req, callback) {
        ...
        if (this._sessionHandler) {
            ...
            let data = {
                raw: this.processValues(req, true, false, true),
                values: this.processValues(req, true, false, false),
            ...
            this._sessionHandler.save(req, this, data, function (error) {
                if (callback != null) {
                    if (error) {
                        //error
                        callback.call(that, req, that, error);
                    } else {
                        //success
                        callback.call(that, req, that, null);
                    }
                }
            });
        } else {
