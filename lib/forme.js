    ...
    _validate(req, values) {
        ...
        .catch(err => {
            ...
            return new Promise((resolve, reject) => {
                req.forme[this._name].first = false;

                return this.store(req)
                .then(() => {
                    //session saved, always resolve
                    resolve(req.forme[this._name].errors);
                })
                .catch(err => {
                    //failed to save session
                    //add error
                    let errors = req.forme[this._name].errors.slice();
                    errors.push(err);
        ...
        if (this._sessionHandler) {
            ...
            .then(session => {
                ...
                for (let index = 0; index < this._inputs.length; index++) {
                    ...
                    reqForm.raw[input._name] = this._findValue(req, input, submit, true, null, values);
                    reqForm.values[input._name] = this._findValue(req, input, submit, false, null, values);
                }
