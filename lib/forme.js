...
class Forme extends FormeContainer {
    ...
    _load() {
        if (this._sessionHandler) {
            ...
            .then(session => {
                //save session into the request
                extend(true, this._request._session, session);

                //copy data from session
                //(todo: perhaps we can just rely in teh session object and get rid of separate container details)
                this._request._errors = this._request._session.errors;
                this._request._first = this._request._session.first;
                this._request._page = this._request._session.page;
