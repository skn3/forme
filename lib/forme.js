...
class Forme {
    ...
    _validate(req, values) {
        ...
        .catch(err => {
            ...
            return new Promise((resolve, reject) => {
                const request = req.forme[this._name];
        ...
        if (this._sessionHandler) {
            ...
            .then(session => {
                ...
                for (let index = 0; index < this._inputs.length; index++) {
                    ...
                    request._raw[input._name] = this._findValue(req, input, submit, true, null, values);
                    request._values[input._name] = this._findValue(req, input, submit, false, null, values);
                }
    ...
    _containsRequest(req) {
        return req !== null && typeof req == 'object' && req.forme !== undefined && req.forme[this._name] !== undefined && req.forme[this._name] instanceof Request;
    }
    ...
    _findValue(req, input, submit, raw, defaultValue, values) {
        ...
        const request = req.forme[input._form._name];
    ...
    _validateNextFormHandler(req, index) {
        ...
        .then(() => ++index == this._validateHandlers.length ? Promise.resolve() : this._validateNextFormHandler(req, index))
        .catch(err => {
