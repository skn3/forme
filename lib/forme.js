...
class Forme extends FormeContainer {
    ...
    _runSubmit(submit, values) {
        ...
        .then(() => this._processSteps(true, [
            ...
            () => !this._requestPage?Promise.resolve():this._requestPage._execute(),//calls page._fooChildren then page._fooSelf
            ...
            () => this._successSelf(),
            () => !this._requestPage?Promise.resolve():this._requestPage._success(),//calls page._fooSelf then page._fooChildren
            () => this._successChildren(),

            //submit handlers
            () => this._submitChildren(),
            () => !this._requestPage?Promise.resolve():this._requestPage._submit(),//calls page._fooChildren then page._fooSelf
            () => this._submitSelf(),

            //trigger action handlers
            () => !this._requestPage?Promise.resolve():this._requestPage._armTriggers(),
            () => this._armTriggers(),
            () => !this._requestPage?Promise.resolve():this._requestPage._fireTriggerActions(),//calls page._fooChildren then page._fooSelf
            () => this._fireTriggerActions(),

            //done handlers
            () => this._doneChildren(),
            () => !this._requestPage?Promise.resolve():this._requestPage._done(),//calls page._fooChildren then page._fooSelf
            () => this._doneSelf(),
        ]))
        ...
        .catch(err => {
            ...
            return this._processSteps(false, [
                ...
                //different order, form first as page might rely on something form provides!
                () => this._failSelf(),
                () => !this._requestPage?Promise.resolve():this._requestPage._fail(),//calls page._fooSelf then page._fooChildren
                () => this._failChildren(),
            ])
    ...
    _runLoad() {
        ...
        this._currentRequest._loading = true;
        return this._currentRequest._load()
        .then(() => {
            this._currentRequest._loading = false;
            this._currentRequest._started = true;
        })
        .catch(err => {
    ...
    _runSave() {
        ...
        if (!this._currentRequest._runReset && this._currentRequest._submit) {
            ...
            this._currentRequest._setCurrentStore(this._buildValues({}));
        }
    ...
    _runReset() {
        //flag reset
        this._currentRequest._halt = true;//must halt otherwise submit handlers could fire after
        this._currentRequest._reset = true;
        this._currentRequest._reload = true;
    ...
    _runNext() {
        ...
        this._currentRequest._next = true;
        this._currentRequest._reload = true;
        this._currentRequest._special = true;
        this._currentRequest._destination = page ? page._destination : null;
    ...
    _runPrev(keepErrors) {
        ...
        this._currentRequest._halt = true;//must halt otherwise submit handlers could fire after
        this._currentRequest._prev = true;
        this._currentRequest._reload = true;
        this._currentRequest._special = true;
        this._currentRequest._destination = page ? page._destination : null;
        this._currentRequest._future = page ? page._name : null;
    ...
    _runFinish(success) {
        ...
        this._currentRequest._visit(this._requestPage);
        ...
        if (!success) {
            ...
            if (this._currentRequest._submit) {
                ...
                //flag invalid
                this._currentRequest._valid = false;
                ...
                if (this._hasPages()) {
                    ...
                    if (this._isFirstPage()) {
                        //cant go back a page
                        this._currentRequest._reload = false;
                    } else {
                        //go back a page
                        this._runPrev(true);
                    }
            ...
            if (!this._currentRequest._runPrev && this._currentRequest._submit) {
                //a good submit!
                this._currentRequest._complete(this._requestPage);
                this._currentRequest._invalidateAfter(this._requestPage);
            }
    ...
    _runShutdown(save) {
        ...
        if (this._currentRequest._submit && !this._currentRequest._reload) {
            this._currentRequest._finished = true;
        }
        ...
        if (!save) {
            ...
            .catch(err => {
                ...
                this._currentRequest._valid = false;
                this._currentRequest._finished = false;
                this._currentRequest._reload = true;
                return Promise.resolve();
            });
        }
    ...
    _build(view, submit, values) {
        return this._buildSteps(view, submit, values, [
            ...
            (view, submit, values) => !this._requestPage?Promise.resolve():this._requestPage._buildLoadHandlers(view, submit, values),//page load handlers

            //build
            (view, submit, values) => this._buildBuildHandlers(view, submit, values),
            (view, submit, values) => !this._requestPage?Promise.resolve():this._requestPage._build(view, submit, values),//page build handlers

            //children
            (view, submit, values) => this._buildChildren(view, submit, values),
            (view, submit, values) => !this._requestPage?Promise.resolve():this._requestPage._buildChildren(view, submit, values),//page children
    ...
    _buildSteps(view, submit, values, steps) {
        //finished building
        this._currentRequest._building = true;
        ...
        .then(() => {
            if (error) {
                ...
                this._currentRequest._building = false;
            }
        })
        .catch(err => {
            this._currentRequest._building = false;
    ...
    _buildRequest(view, submit, values) {
        ...
        this._currentRequest._first = !submit && !this._currentRequest._hasVisitedPage();
    }
    ...
    _buildTemplateVars(options) {
        return Object.assign(super._buildTemplateVars(options), {
            ...
            first: this._currentRequest._formFirst && this._requestFirst,//both must be first!
            errors: this._currentRequest._fetchOrphanedErrors(),
        });
    }
    ...
    _execute() {
        //override as we manually call self/child methods in teh form execution steps
    ...
    _pipeLostError(error) {
        ...
        this._currentRequest._addError(null, error);
    }
        ...
        if (submit) {
            ...
            if (first) {
                ...
                if (raw) {
                    ...
                    const result = this._currentRequest._values[valueName];
                    return result === undefined?null:result;
                }
            }
        }
    ...
    _readValueFromStore(valueName) {
        const store = this._currentRequest._getCurrentStore();
    ...
    _fetchErrors() {
        return this._currentRequest._fetchCurrentErrors();
    }
    ...
    _makeAction() {
        ...
        //add the token
        query[this._tokenField] = this._currentRequest._token;
    ...
    //private get methods
    _getNamedValue(name, unsafe=false, defaultValue=null) {
        if (name === null || name.length === 0) {
            return defaultValue;
        }
        ...
        if (this._isBuilding()) {
            ...
            if (input === null) {
                if (unsafe) {
                    return this._currentRequest._getValue(name, defaultValue);
                } else {
                    return defaultValue;
                }
            }
            ...
            const value = this._readValue(name, this._requestFirst, this._currentRequest._submit, false, true, input?input._permanentValue:null, input?input._overrideValue:null, null, null);
            return input._getProcessedValue(value);
        } else {
            //form has already been built, so we know that the value will exist in the request!
            return this._currentRequest._values[name];
        }
    }
    ...
    _catchError(err) {
        ...
        if (err && err.message.length && !(err instanceof FormeValidationError)) {
            if (constants.dev) {
                ...
                //to container
                this.addError('[development] '+err.toString());
            } else {
                //production
                this.addError(this._createError('Unhandled Error'));
            }
        }
    ...
    getNamedErrors(name) {
        //filter by name
        return this._currentRequest._fetchNamedErrors(name);
    }

    getNamedValue(name, defaultValue=null) {
        if (this.callingActiveMethod('getNamedValue')) {
            return this._getNamedValue(name, false, defaultValue);
        }
    }

    getUnsafeNamedValue(name, defaultValue=null) {
        if (this.callingActiveMethod('getNamedValue')) {
            return this._getNamedValue(name, true, defaultValue);
        }
    }
