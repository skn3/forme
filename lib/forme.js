...
const {FormeConfigurableMethod, FormeConfigurableOverride, FormeConfigurableParam, FormeConfigurableBool, FormeConfigurableInt, FormeConfigurableFloat, FormeConfigurableString, FormeConfigurableObject, FormeConfigurableArray, FormeConfigurableCallbacks, FormeConfigurableStrings} = require('./configurable');
...
class Forme extends FormeContainer {
    constructor(details) {
        //get correct details
        let name, info, driverClass;
        if (typeof details === 'object') {
            name = details.name || undefined;
            info = details;
            driverClass = details.driver || defaultDriverClass;
        } else {
            name = details;
    ...
    static buildConfigurableMethods() {
        return Object.assign(super.buildConfigurableMethods(), {
            ...
            //form.page(value) -> renamed to "pages"
            pages: new FormeConfigurableMethod('page', [
                new FormeConfigurableOverride([
                    new FormeConfigurableParam('page', true),
                ], true),
            ]),
    ...
    _clone(override) {
        //create copy
        const clone = new this._driverClass.formClass();
    ...
    _addPageContainer(name, callback) {
        ...
        const page = new this._driverClass.pageClass(this, name, callback);
    ...
    template() {
        if (utils.call.check.active(this, 'form.template()')) {
            ...
            for (let input of this._inputs) {
                utils.group.addGroup(template.input, input._outputName(), input._group, input.template());
            }
    ...
    method(method, action) {
        ...
        throw new FormeError(`unsupported method '${method}'`);
    }
    ...
    page() {
        //todo: clean up this fugly code!
        if (utils.call.check.not.active(this, 'form.page()')) {
            if (arguments.length >= 1) {
                if ((arguments.length === 2 && arguments[1] === true) || (Array.isArray(arguments[0]) && arguments[0].every(item => typeof item === 'string'))) {
                    //add page location
                    ...
                    if (this._pageAllowed === false) {
                        ...
                        if (Array.isArray(arguments[0])) {
                            //multiple
                            for(let page of arguments[0]) {
                                if (typeof page === 'object') {
                                    //add and configure
                                    return this._addPageContainer(page.name, null).configure(page);
                                } else {
                                    //just add with name
                                    return this._addPageContainer(page, null);
                                }
                            }
                        } else {
                            //single
                            if (typeof arguments[0] === 'object') {
                                //add and configure
                                return this._addPageContainer(arguments[0].name, null).configure(arguments[0]);
                            } else {
                                //just add with name
