...
class Forme {
    ...
    _buildTemplate(reqForm) {
        ...
        for (let inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
            ...
            if (typeof template.input[input._groupName] == 'undefined') {
                ...
                if (!(template.input[input._groupName] instanceof Array)) {
                    //convert to array
                    template.input[input._groupName] = [template.input[input._groupName]];
                }

                //add
                template.input[input._groupName].push(buildInput);
            }
        }
    ...
    validate(req) {
        ...
        return this._process(req, true).then(function() {
            ...
            //save session, wrap this in a promise so we can convert a reject(catch) into a resolve
