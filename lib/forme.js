...
class Forme extends FormeContainer {
    ...
    _build(submit, values) {
        ...
        this._request._identifier = this._buildIdentifier();
    ...
    _buildTemplate() {
        ...
        for (let input of this._inputs) {
            ...
            utils.group.addGroup(template.input, buildInput.alias, input._group, buildInput);
        }
    ...
    _processNextValidateHandler(index) {
        ...
        const lookup = {};
        const state = {
            values: this._fetchValues(false, true, false, true, lookup, null),
        };
        const oldValues = this._fetchValues(false, false, false, true, null, null);
    ...
    _buildGroupStructure(structure) {
    ...
    _fetchValues(secure, group, raw, store, lookup, values) {
        ...
        if (group) {
            this._buildGroupStructure(values);
        }
    ...
    page() {
        if (utils.call.check.inactive(this, 'form.page()')) {
            if (this._pageAllowed === false) {
                throw new Error('cant call form.page() after form.pages()');
            } else {
                this._pageAllowed = true;

                //static or dynamically added?
                if (arguments.length == 2) {
                    //dynamic page (with callback)
                    this._addPageContainer(arguments[0], arguments[1]);

                    //chain
                    return this;
                } else if (arguments.length == 1) {
                    //static page (with path/url)
    ...
    pages() {
        //add steps for form (allows each page of a form to be a separate form)
        if (utils.call.check.inactive(this, 'form.pages()')) {
            if (this._pageAllowed === true) {
                throw new Error('cant call form.pages() after form.page()');
            } else {
                for(let locations of arguments) {
                    if (Array.isArray(locations)) {
                        for (let location of locations) {
                            this._addPageLocation(location);
                        }
                    } else {
                        this._addPageLocation(locations);
                    }
                }
            }
        }
