...
class Forme extends FormeContainer {
    ...
    _finish(success) {
        if (!success) {
            ...
            if (this._request._submit) {
                ...
                //determine if we have a page we can _reload with
                if (this._hasPages()) {
            ...
            //only if its a submit though...
            if (this._request._submit) {
                this._request._visit();
            }

            //shutdown and save the form in certain circumstances. We don't need to save the page if the form has completed or if its the first page load.
            return this._shutdown((!this._request._pageFirst && !this._request._submit) || this._pages !== null)
            .then(() => {
    ...
    _buildRequest(view, submit) {
        ...
        //set first states
        this._request._pageFirst = !submit && ((this._request._page === null && this._request._formFirst) || (this._request._page !== null && !this._request._store.visited[this._request._current]));
        this._request._formFirst = this._request._formFirst && this._request._pageFirst;
    }
    ...
    _buildValues(view, submit, values) {
        //read values
        for (let input of this._inputs) {
            this._request._raw[input._name] = this._readInputValue(input, this._request._pageFirst, submit, true, false, null, values);//cant read from store
            this._request._values[input._name] = this._readInputValue(input, this._request._pageFirst, submit, false, !!submit, null, values);//can only read from the store when NOT submitting
        }
    ...
    _readValue(valueName, first, submit, raw, store, permanentValue, defaultValue, values) {
    ...
    _readInputValue(input, first, submit, raw, store, defaultValue, values) {
        ...
        return this._readValue(input._name, first, submit, raw, store, input._permanentValue, !submit && first?input._defaultValue || defaultValue:defaultValue, values);
    }
    ...
    _getInputNameValue(inputName) {
        ...
        if (this._isBuilding()) {
            ...
            return input._processWithValue(this._readValue(inputName, this._request._pageFirst, this._request._submit, false, true, input?input._permanentValue:null, null, null), false);
        } else {
    ...
    _catchError(err) {
        ...
        if (err !== undefined && err.message.length && !(err instanceof FormeError) && !(err instanceof FormeInputError)) {
            if (constants.dev) {
                //development mode
                //to console
    ...
    value() {
        if (utils.call.check.not.inactive(this, 'form.value()')) {
            if (arguments.length === 1) {
                ...
                return this._getInputNameValue(input._name);

            } else if (arguments.length === 2) {
                //set value
                const input = this._findInput(arguments[0]);
                if (input !== null) {
                    this._setInputNameValue(input._name, arguments[1]);
                }
