...
class Forme {
    ...
    _buildTemplate(reqForm) {
        ...
        for (let inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
            ...
            this._addToGroupStructure(template.input, buildInput.alias, input._group, buildInput);
        }
    ...
    _submitNextFormHandler(req, index) {
        ...
        //current submit
        return utils.promise.result(that._submitHandlers[index].call(that, req, that))
        .then(function(){
            //next submit
    ...
    _catchError(req, err) {
        ...
        if (typeof err != 'undefined' && !(err instanceof FormeError) && !(err instanceof FormeInputError)) {
            if (dev) {
                //development mode
                //to console
        ...
        for (let index = 0;index < this._inputs.length;index++) {
            ...
            if (!secure || !input._secure) {
                ...
                if (!group) {
                    ...
                    //add to structure
                    this._addToGroupStructure(values, alias, input._group, value);
