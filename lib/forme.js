...
const {
    ...
    FormeConfigurableExportIntOrNull,
...
class Forme extends FormeContainer {
    ...
    _buildConfigurableMethods() {
        return Object.assign(super._buildConfigurableMethods(), {
            ...
            page: new FormeConfigurableMethod('page', [
                ...
                //form.page(name) (this should come as last override so it doesn't swallow the array/object versions with its default value)
                new FormeConfigurableOverride([
            ...
            externalPage: new FormeConfigurableMethod('externalPage', [
                //form.page(name)
                new FormeConfigurableOverride([
    ...
    get pageIndex() {
        if (this._pages) {
            return this._findPageIndex(this._currentRequest._currentPageName)
        } else {
            return -1;
        }
    }
    ...
    _runWrap(submit, storage) {
        ...
        return new Promise((resolve, reject) => {
            ...
            //prepare
            form._currentRequest = new FormeRequest(form, submit);
    ...
    _runView(values) {
        ...
        .then(() => this._build(values)) //immediate calls are 1) _buildStart() 2) _buildRequest() ... (so basically the request gets built early!)
    ...
    _runLoad() {
        ...
        .catch(err => {
            //failed to load
            //this._catchError(err);//todo: form might reset so unless we have code to extract the errors in user code somehow...
    ...
    _runNext() {
        ...
        this._currentRequest._destination = page ? page._destination : null;
        this._currentRequest._setFuturePageName(page)
    }
    ...
    _runFinish(success) {
        ...
        if (!success) {
            ...
            if (this._currentRequest._submit) {
                ...
                //did the form reset?
                if (this._currentRequest._reset) {
                    //form has been reset so lets flag a reload!
                    //this._currentRequest._flagReload();
                } else {
                    //is the form paged?
                    if (this._hasPages()) {
    ...
    _build(external) {
        //replaces built in method fo base! This is because we need to add a ton more things into the build timeline!
        return this._buildStart(external)
        .then(() => this._buildSteps(external, [
            //prepare request
            () => this._buildRequest(external),

            //import page info
            () => this._buildImportPage(external),

            //load handlers
            () => this._buildLoadHandlers(external),

            //build custom callbacks
            () => this._buildBuildHandlers(external),

            //build out children
            () => this._buildChildren(external),

            //run any modifications to the built structure! (prior to reading data in the read step)
            () => this._buildModify(external),

            //read (this has to come after _buildChildren() because we need to make sure all inputs exist!)
            () => this._buildRead({ raw:{}, values:{} }, external),

            //finalise (this lets us do things in the "building" state knowledge that we have a fully formed set of elements with data read)
            () => this._buildFinalise(external),
        ]))
    ...
    _buildSteps(external, steps) {
        ...
        return steps.reduce((prev, curr) => {
            ...
            return prev.then(() => !error?curr.call(this, external):Promise.resolve())
    ...
    _buildRequest(external) {
        //override the current page that was default set in request._load()
        this._currentRequest._setPage(this._findPageDestination(this._currentDriver.url()), true);//override with page for current url (or keep whatever is currently set if result was null)
        this._currentRequest._setPage(this._findPage(this._currentDriver.request(this._pageField)), true);//override with get/post page (or keep whatever is currently set if result was null)
        ...
        this._currentRequest._setFuturePageName(this._currentRequest._currentPageName);
        ...
        if (!this._currentRequest._submit && !this._currentRequest._hasVisitedCurrentPage()) {
    ...
    _buildReadFinish(out, external) {
        //store these in the request once the entire read operation has finished!
        this._currentRequest._raw = out.raw;
        this._currentRequest._values = out.values;
    ...
    _buildValuesSelf(options, depth=0) {
        if (options.pages) {
            ...
            const selfValues = super._buildValuesSelf(options, depth);
    ...
    _processJourney() {
        ...
        if (this._requestPage) {
            ...
            if (valid && this._requestPage._destination !== null) {
                ...
                if (!utils.url.comparePaths(currentUrl, this._requestPage._destination)) {
                    valid = false;
                }
            }
            ...
            if (valid) {
                ...
                const pageIndex = this._findPageIndex(this._requestPage);
                if (pageIndex === -1) {
                    //erm that page does not exist (this should probably not get here, but we put it in as a precaution!)
                    valid = false;
                } else if (pageIndex > 0) {
                    ...
                    for (let index = 0; index < pageIndex; index++) {
                        ...
                        if (!this._currentRequest._hasValidPageState(page)) {
        ...
        if (valid) {
            ...
            if (lastViablePage) {
                this._currentRequest._setFuturePageName(lastViablePage._name);
    ...
    _invalid() {
        //dont need to call ancestors as we are the master of all :D
    ...
    _executeReadHandler(handler, value, state) {
        return Promise.resolve(handler.call(this, this.form, value, state));
    }
    ...
    _executeGetterHandler(handler, value) {
        return handler.call(this, this._form, value);
    }
    ...
    _readValueFromEverywhere(valueName, first, submit, raw, store, permanentValue, overrideValue, defaultValue, external) {
        ...
        if (external) {
            const result = external[valueName];
            if (result !== undefined) {
                return result;
            }
        }
    ...
    _findPageIndex(page) {
        if (this._pages) {
            ...
            for (let index = 0; index < this._pages.length; index++) {
                if (this._pages[index]._name === name) {
                    return index;
                }
            }
        }
    ...
    _findPageDestination(destination) {
        if (this._pages && this._pages.length) {
            return this._pages.find(page => utils.url.containsQuery(destination, page._destination) && utils.url.comparePaths(destination, page._destination)) || null;
        }
        return null;
    }
    ...
    _findPreviousPage() {
        if (this._pages) {
            if (this._requestPage === null) {
                ...
                for (let index = 0; index < this._pages.length; index++) {
                    const page = this._pages[index];
                    if (page._name === this._currentRequest._currentPageName) {
                        if (index === 0) {
                            return null;
                        } else {
    ...
    _makeIdentifier() {
        ...
        if (this._identifier !== null) {
            ...
            if (this._requestPage !== null) {
                ...
                return '__forme_identifier_page__' + this._currentRequest._currentPageName;
            } else {
    ...
    _makeDestination() {
        ...
        //calculate details
        const page = this._pageAllowed !== false && this._pages !== null && !this._isFirstPage(this._currentRequest._futurePageName);
        let destination = this._currentRequest._destination === null?this._currentDriver.url():this._currentRequest._destination;

        return this._makeUrl(destination, this._currentRequest._token, page?this._currentRequest._futurePageName:false);
    }
    ...
    _isFirstPage(page) {
        if (!this._hasPages()) {
            ...
            if (!page) {
                return this._findPageIndex(this._currentRequest._currentPageName) === 0;
            } else {
    ...
    view(storage, values=null) {
        if (this.callingOperationMethod('view')) {
            ...
            return this._runWrap(false, storage)
    ...
    execute(storage, values=null) {
        if (this.callingOperationMethod('execute')) {
            //wrap it in clone of form
            return this._runWrap(true, storage)
            .then(form => form._runSubmit(values));
        }
    }
    ...
    getUrl(page=null) {
        if (this.callingStartedMethod('getUrl')) {
            if (!this._hasPages()) {
                //form doesnt have pages so get the url from the driver directly!
                return this._currentDriver.url();
            } else {
                //form has pages
                if (!page) {
                    //current page dictates the url
                    return this._currentRequest._page._url;
                } else {
                    //page was passed so we need to see if it actually exists?
                    page = this._findPage(page);
                    if (!page) {
                        //page doesnt exist, get base url!
                        return this._currentDriver.url();
                    } else {
                        //page has complete control of building its own url! (this allows us to abstract out to internal vs external pages)
                        return page._url;
                    }
                }
            }
    ...
    getPageWithName(name) {
        if (this._pages && this._pages.length && name) {
            return this._pages.find(page => page._name === name) || null;
        }
        return null;
    }
    ...
    hasVisited(page) {
        ...
        if (this.callingStartedMethod('hasVisited')) {
            if (!this._hasPages()) {
                return false;
            } else {
                if (page === undefined) {
    ...
    hasCompleted(page) {
        ...
        if (this.callingStartedMethod('hasCompleted')) {
    ...
    //force methods
    forceFail() {
        //forces this to be invalid
        if (this.callingStartedMethod('forceFail')) {
...
module.exports.FormeConfigurableIntOrNull = FormeConfigurableIntOrNull;
...
module.exports.FormeConfigurableExportIntOrNull = FormeConfigurableExportIntOrNull;
