...
class FormeConfigurableMethod extends FormeConfigurableMethodBase {
    constructor(method, overrides, exporter=null, priority=null, sourceProperty=null) {
        //todo: remove sourcePriority crud?
        super();

        this.method = method;
        this.overrides = overrides;
        this.exporter = exporter;
        this.priority = priority === null || priority < lowestPriority?lowestPriority:priority;
    ...
    clone(sourceProperty=null) {
        ...
        return new this.constructor(this.method, !this.overrides?undefined:this.overrides.map(override => override.clone()), !this.exporter?null:this.exporter.clone(), this.priority, sourceProperty || this.sourceProperty);
    }
    ...
    call(source, input) {
        ...
        for(let override of this.overrides) {
            try {
                ...
                if (!override.checkZero(input)) {
                    ...
                    if (override.checkSingle(input)) {
                        ...
                        if (param.checkValueType(input)) {
                            ...
                            //extract value from object, make sure that the override's strict policy matches the input
                        ...
                        //did not match single param, so we need to check incase the override is flagged as requiring strict input (eg cant contain ANY other properties other then specified)
                ...
                //at this point we should have a built array where ALL values have been validated and converted in to a flat array that matches the length of the expected params
...
class FormeConfigurableOverride extends FormeConfigurable {
    ...
    checkZero(input) {
        ...
        return !!input && (!this.params || this.params.length === 0);
    }
    ...
    checkStrictInput(input) {
        //so we need to check incase the override is flagged as reqiuring strict input (eg cant contain ANY other properties other then specified)
        //this is because we might have .foo(name, value) and .foo({name: '', value: '', other: ''}) overrides. In this scenario we should set the .foo(name, value)
        //as strict, otherwise it will eat up the second override!

        //innocent until proven guilty
        if (this.strict && typeof input === 'object' && !Array.isArray(input)) {
...
class FormeConfigurableIntOrNull extends FormeConfigurableParam {
    //api
    convert(value) {
...
class FormeConfigurableString extends FormeConfigurableParam {
    ...
    checkValueType(value) {
        return typeof value === 'string' || typeof value === 'number';//allow number
    }
}
...
class FormeConfigurableStringOrNull extends FormeConfigurableParam {
    ...
    checkValueType(value) {
        return value === null || typeof value === 'string' || typeof value === 'number';//allow number
    }
}
...
class FormeConfigurableStrings extends FormeConfigurableParam {
    ...
    checkValueType(value) {
        return typeof value === 'string' || typeof value === 'number' || (Array.isArray(value) && value.findIndex(callback => typeof callback !== 'string' && typeof callback !== 'number') === -1);
    }
}
...
class FormeConfigurableStringsOrNull extends FormeConfigurableParam {
    ...
    checkValueType(value) {
        return value === null || typeof value === 'string' || typeof value === 'number' || (Array.isArray(value) && value.findIndex(value => typeof value !== 'string' && typeof callback !== 'number') === -1);
    }
}
...
class FormeConfigurableExportArrayKeyValues extends FormeConfigurableExportArray {
    constructor(property, keyProperty, valueProperty) {
        super(property);
        this.keyProperty = keyProperty || 'key';
        this.valueProperty = valueProperty || 'value';
    }
    ...
    export(source) {
        ...
        for(let child of value) {
            if (child !== undefined && child !== null && typeof child !== 'object' && child.hasOwnProperty(this.keyProperty) && child.hasOwnProperty(this.valueProperty)) {
                out.push({
                    [this.keyProperty]: child[this.valueProperty],
                });
            }
        }
...
class FormeConfigurableExportIntOrNull extends FormeConfigurableExport {
    //api
    export(source) {
        const value = source[this.property];
        if (value === undefined || value === null) {
...
module.exports = {
    ...
    FormeConfigurableIntOrNull: FormeConfigurableIntOrNull,
    ...
    FormeConfigurableExportIntOrNull: FormeConfigurableExportIntOrNull,
    ...
    FormeConfigurableExportArrayKeyValues: FormeConfigurableExportArrayKeyValues,
