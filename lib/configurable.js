            ...
            try {
                ...
                if (!override.checkZero(input)) {
                    ...
                    if (override.checkSingle(input)) {
                        ...
                        //multiple params, so we need to check incase the override is flagged as requiring strict input (eg cant contain ANY other properties other then specified)
...
class FormeConfigurableOverride extends FormeConfigurable {
    ...
    checkStrictInput(input) {
        ...
        if (this.strict) {
            if (typeof input === 'object' && !Array.isArray(input)) {//ignore arrays because they will generally always have keys not included because of number index!
