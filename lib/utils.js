...
function arraysOverlap(array1, array2) {
    for(let index1 = 0; index1 < array1.length; index1++) {
        if (array2.indexOf(array1[index1]) !== -1) {
            return true;
        }
    }
    return false;
...
//tag functions
function compareTagWildcard(tags1, tags2) {
    return tags1 === '*' || tags2 === '*' || (Array.isArray(tags1) && tags1.indexOf('*') !== -1) || (Array.isArray(tags2) && tags2.indexOf('*') !== -1);
}

function compareTagsAnd(tags1, tags2, wildcard=true) {
    //handle wildcards
    if (wildcard && compareTagWildcard(tags1, tags2)) {
        return true;
    }

    if (Array.isArray(tags1)) {
        //multiple, so have to match all tags via or
        for(let tag of tags1) {
            if (!compareTagsOr(tag, tags2, false)) {//dont need to process wildcards as we already have
                return false;
            }
        }
        return true;
    } else if (typeof tags1 === 'string') {
        //single (so same as calling or)
        return compareTagsOr(tags1, tags2, false);//dont need to process wildcards as we already have
    }
...
function compareTagsOr(tags1, tags2, wildcard=true) {
    //handle wildcards
    if (wildcard && compareTagWildcard(tags1, tags2)) {
        return true;
    }
    ...
    if (Array.isArray(tags1)) {
        if (Array.isArray(tags2)) {
            //exact match
            return arraysOverlap(tags1, tags2);
        } else if (typeof tags2 === 'string') {
            //string match (+allow wildcard)
            return tags1.indexOf(tags2) !== -1;
        }
    } else if (typeof tags1 === 'string') {
        if (Array.isArray(tags2)) {
            return tags2.indexOf(tags1) !== -1;
        } else if (typeof tags2 === 'string') {
            return tags1 === tags2;
        }
    }
